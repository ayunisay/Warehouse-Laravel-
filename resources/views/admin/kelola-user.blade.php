@include('admin.admin-layout.header')
    <div class="container mx-auto mt-8 flex-grow">
        ...
        <div class="bg-white shadow-md rounded-lg p-6 mb-8">
            ...
            <form action="{{ route('editUserSubmit', ['id' => $editUser->id]) }}" method="POST">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="name" name="name" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Masukkan nama" value="{{ $editUser->name }}" required>
                    </div>
                    <div>
                        ...
                        <input type="email" id="email" name="email" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Masukkan email" value="{{ $editUser->email }}" required>
                    </div>
                    <div>
                        <label for="password" class="block text-gray-600 font-medium mb-2">
                            Password (kosongkan jika tidak ingin mengubah)
                        </label>
                        <input type="password" id="password" name="password" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Masukkan password baru">
                    </div>
                    <div>
                ...
                <button type="submit" class="mt-4 bg-blue-600 text-white px-6 py-2 mr-3 rounded-lg hover:bg-blue-700 transition duration-300">
                    Update User
                </button>
